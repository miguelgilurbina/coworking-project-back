...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                .csrf().disable()  // Desactivar CSRF para pruebas
                .authorizeRequests()
                .antMatchers(HttpMethod.POST, "/**").permitAll() // Permitir POST sin autenticación para pruebas
                .antMatchers("/**").permitAll() // Permitir todos los otros métodos
                .anyRequest().authenticated() // Autenticar cualquier otra solicitud
                .and()
                .httpBasic(); // Puede cambiarse por otro método de autenticación
    }
}
